...
const CACHE_DURATION = 30 * 60 * 1000; // 30 分钟缓存（Avatar 列表变化不频繁）
 ...
 */
export const getAvatars = functions
  .region("us-west1")
  .https.onCall(
    ...
    // 1. 验证用户是否已登录
      ...
      // 3. 检查缓存（性能优化）
        ...
        try {
          response = await fetch(heygenApiUrl, {
            method: "GET",
            headers: {
          ...
          if (fetchError instanceof Error && fetchError.name === "AbortError") {
            functions.logger.error("❌ HeyGen API 请求超时（10秒）");
            throw new functions.https.HttpsError(
              "deadline-exceeded",
              "HeyGen API request timeout (10s)"
            );
          }
          throw fetchError;
        }
        ...
        // 移除详细响应日志（性能优化，减少日志输出）
        // 只在调试时打印前 200 个字符
        if (process.env.DEBUG === "true") {
          functions.logger.info(
            "📦 HeyGen API 原始响应（前200字符）:",
        ...
        if (result && typeof result === "object") {
          ...
          if ("data" in resultObj && resultObj.data) {
            ...
            "avatars" in resultObj && Array.isArray(resultObj.avatars)
          ) {
          // 备用格式：{ avatars: [...] }
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        // eslint-disable-next-line max-len, @typescript-eslint/no-explicit-any
          ...
          // 处理不同的字段名变体
            ...
            // 记录所有可用的字段名（用于调试）
            // 只检查前 10 个 Avatar，避免日志过多（性能优化）
            const allKeys = Object.keys(avatar);
            if (index < 10 &&
            !allKeys.includes("preview_url") &&
            ...
            // 只在调试模式下或前几个 Avatar 打印详细日志（性能优化）
            if (!previewUrl) {
              // 只记录前 5 个缺少预览 URL 的 Avatar，避免日志过多
              if (index < 5) {
                functions.logger.warn(
            ...
            // 移除每个 Avatar 的成功日志，只在最后打印汇总（性能优化）
